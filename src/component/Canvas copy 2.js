...
export default class CanvasComponent extends Component {
  ...
  constructor(props) {
    super(props);
    this.state = {
      width: props.isMobile ? window.innerWidth/1.3 : window.innerWidth/2.3,
    ...
    // if(!this.state.isEdit) {
      ...
      if (this.mouse.button === 1) { // is mouse button down
        ...
        if(this.orderNumber > 0) {
          this.length = Math.round(this.getLength(this.points[Math.abs(drawingLastPointIndex-1)].x, this.points[Math.abs(drawingLastPointIndex-1)].y, x, y));
          this.middlePoint = this.getMiddlePoint(this.points[Math.abs(drawingLastPointIndex-1)].x, this.points[Math.abs(drawingLastPointIndex-1)].y, x, y);
          this.putText(this.length , this.zoomedX(this.middlePoint.x), this.zoomedY(this.middlePoint.y), 'red');
        }

        //draw angle
        if(this.orderNumber >= 2) {
            this.arc = (this.getArc(this.points[Math.abs(drawingLastPointIndex-2)].x, this.points[Math.abs(drawingLastPointIndex-2)].y, this.points[Math.abs(drawingLastPointIndex-1)].x, this.points[Math.abs(drawingLastPointIndex-1)].y, this.points[Math.abs(drawingLastPointIndex-1)].x, this.points[Math.abs(drawingLastPointIndex-1)].y, x, y)).toFixed(1);
            this.putText(this.arc + "°", this.zoomedX(this.points[Math.abs(drawingLastPointIndex-1)].x), this.zoomedY(this.points[Math.abs(drawingLastPointIndex-1)].y), 'blue')
        }
      ...
      this.drawDot(this.points[i], 'yellow');
        if(i > 0) {
          ...
          {            
            ...
            this.putText(this.length , this.zoomedX(this.middlePoint.x), this.zoomedY(this.middlePoint.y), 'red'); // write text in a middle point
            if(i >= 2) {
              ...
              this.putText(this.arc + "°", this.zoomedX(this.points[i-1].x), this.zoomedY(this.points[i-1].y), 'blue')
            }
          this.context.stroke();
          this.context.closePath();
        }                 
  ...
  handleChangeAngleModal = () => {
    ...
    var oldAngle = this.state.oldAngle
  ...
  render() {
    return (
      <div className="canvasContainer">
        ...
        <canvas ref='canvas' id='drawingCanvas' width={this.state.width} height={this.state.height} onMouseUp={this.mouseUp.bind(this)} onMouseDown={this.mouseDown.bind(this)} onDoubleClick={this.mouseDbl.bind(this)} onMouseMove={this.mouseMove.bind(this)} onContextMenu={this.mouseRightClick.bind(this)} onWheel={this.handleMouseWheel.bind(this)} />
      </div>
    );
  }
}