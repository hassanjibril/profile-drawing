...
export default class CanvasComponent extends Component {
  ...
  addPoint(x, y) {
    var isInValidPointIndex = -1;
    var isInValidLineIndex = -1;
    var x = Math.round(x);
    var y = Math.round(y);
    for(var i=0; i<this.points.length; i++) {
      if(this.points[0].x != undefined && this.getLength(x, y, this.points[i].x, this.points[i].y) <= 50) {
        isInValidPointIndex = i;
      }
      if(i > 1 && this.intersect(this.points[i-2].x, this.points[i-2].y, this.points[i-1].x, this.points[i-1].y, this.points[this.points.length-1].x, this.points[this.points.length-1].y, x, y) == 1) {
        isInValidLineIndex = i;
      }
    }
    if(isInValidPointIndex == -1 && isInValidLineIndex == -1) {
      ...
      console.log(isInValidPointIndex, isInValidLineIndex)
    }
  }

  /* detect if two lines are intersected */
  ...
  drawAngle(i) {
    ...
    // draw arc for rounding of edges
    // this.context.save();
    // console.log(a1*180/Math.PI, a2*180/Math.PI)
    // if(-90 < a1*180/Math.PI && a1*180/Math.PI < 90 && -90 < a2*180/Math.PI && a2*180/Math.PI < 90) {
    //   console.log("www1")
    //   x1 = this.zoomedX(this.points[i-1].x + radius);
    //   y1 = this.zoomedY(this.points[i-1].y + radius*dy1/dx1);
    //   x2 = this.zoomedX(this.points[i-1].x + radius);
    //   y2 = this.zoomedY(this.points[i-1].y + radius*dy2/dx2);
    //   cpx = this.zoomedX(this.points[i-1].x);
    //   cpy = this.zoomedY(this.points[i-1].y);
    // }
    // if(-90 < a1*180/Math.PI && a1*180/Math.PI < 90 && -90 < a2*180/Math.PI && a2*180/Math.PI < 90) {
    //   console.log("www2")
    //   x1 = this.zoomedX(this.points[i-1].x - radius);
    ...
    //   cpy = this.zoomedY(this.points[i-1].y);
    // }
    // if(0 < a1*180/Math.PI && a1*180/Math.PI < 180 && 0 < a2*180/Math.PI && a2*180/Math.PI < 180) {
    //   console.log("www3")
    //   x1 = this.zoomedX(this.points[i-1].x + radius);
    //   y1 = this.zoomedY(this.points[i-1].y + radius*dy1/dx1);
    //   x2 = this.zoomedX(this.points[i-1].x - radius);
